...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['_action'] ?? '') === 'add') {
    ...
    if (!$vid || !$type) {
        redirect('/fleetflow/modules/maintenance.php', 'Select a vehicle and service type.', 'error');
    }

    $pdo->prepare(
        "INSERT INTO maintenance_logs (vehicle_id, service_type, description, cost, service_date, technician, status) VALUES (?,?,?,?,?,?,?)"
    )->execute([$vid, $type, $desc, $cost, $date, $tech, $stat]);
    ...
    redirect('/fleetflow/modules/maintenance.php', 'Maintenance log added.');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['_action'] ?? '') === 'complete') {
    ...
    redirect('/fleetflow/modules/maintenance.php', 'Service completed. Vehicle released.');
}
...
<div class="card">
    <div class="table-responsive">
        <table>
            <thead>
                <tr><th>Vehicle</th><th>Service Type</th><th>Description</th><th>Cost</th><th>Date</th><th>Technician</th><th>Status</th><th>Action</th></tr>
            </thead>
            <tbody>
            <?php if (empty($logs)): ?>
            ...
            <tr>
                ...
                <td>
                    <?php if (hasRole('fleet_manager','safety_officer') && $l['status'] !== 'Completed'): ?>
