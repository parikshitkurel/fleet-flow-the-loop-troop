...
requireLogin();
// RBAC: dispatcher + safety_officer have no access to fuel/expense data
if (hasRole('dispatcher','safety_officer')) {
    header('Location: /fleetflownew/dashboard.php?err=access');
    exit;
}
$canWrite = hasRole('admin','fleet_manager','financial_analyst');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['_action'] ?? '') === 'add_fuel') {
    ...
    if (!$vid || $liters <= 0 || $cpl <= 0) {
        redirect('/fleetflownew/modules/fuel.php', 'Fill all required fields.', 'error');
    }
    $pdo->prepare(
    ...
    redirect('/fleetflownew/modules/fuel.php', 'Fuel expense logged.');
}
...
$topbarConfig = [
    'tableId' => 'fuelTable',
    'sorts'   => ['1:desc' => 'Date ↓', '1:asc' => 'Date ↑', '2:desc' => 'Liters ↓', '4:desc' => 'Cost ↓'],
...
<div class="card">
    ...
    <div class="table-responsive">
        <table id="fuelTable">
            ...
            <tr data-searchable="<?= e("{$ex['license_plate']} {$ex['model']} {$ex['expense_date']} {$ex['station']}") ?>"
                data-groupVehicle="<?= e($ex['license_plate']) ?>">
                <td>
